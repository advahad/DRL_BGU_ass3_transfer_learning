...
class AlgorithmParams:
    def __init__(self, env, render, max_episodes, max_steps, discount_factor, policy_learning_rate, learning_rate_decay,
                 solved_th):
        self.env = env
        self.render = render
...
def train_modell(policy, state_value_network, network_params, algorithm_params, LOGS_PATH, MODEL_PATH, save_model = False):
...
def train_mountain_car_model(policy, state_value_network, network_params, algorithm_params, bin_values,
    ...
    with tf.Session() as sess:
        ...
        for episode in range(algorithm_params.max_episodes):
            ...
            for step in range(algorithm_params.max_steps):
                ...
                algorithm_params.policy_learning_rate = decay_learning_rate(algorithm_params.policy_learning_rate, episode,
                                                                            algorithm_params.learning_rate_decay)
...
def train_mc(policy, state_value_network, network_params, algorithm_params, bin_values,
                             LOGS_PATH, MODEL_PATH = None, save_model = False):
    saver = tf.train.Saver()
    summary_writer = summary_util.init(LOGS_PATH)
    start = time.time()
