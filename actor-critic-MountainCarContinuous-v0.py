...
def process_state(state):
    return state
    # discrete_state = np.array([car_pos_bins.cont_to_discrete(state[0]), car_pos_bins.cont_to_discrete(state[1])])
    # discrete_state = discrete_state.reshape([1, state_size])
...
def get_action_discrete(action):
    discrete_action = action_bins.cont_to_discrete(action)
    return discrete_action


def get_action_cont(idx):
...
#         self.mapping = create_mapping(arr)
...
with tf.Session() as sess:
    ...
    for episode in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            # calc advantage
            V_s = sess.run(state_value_network.value_estimate, {state_value_network.state: process_state(state)})
            ...
            # update V network
            state_value_feed_dict = {state_value_network.state: process_state(state),
            ...
            feed_dict = {policy.state: process_state(state),
                         policy.A: td_error * i,
